"""
Multi-GPU Distributed Data Parallel (DDP) Training Loop
This file provides a TrainLoopDDP class that extends the original TrainLoop
with multi-GPU training capabilities using PyTorch DistributedDataParallel.
"""
...
class TrainLoopDDP:
    """
    Training loop with Distributed Data Parallel (DDP) support for multi-GPU training.
    """
    def __init__(self, args, train_platform, model, diffusion, data):
        ...
        # Use DDP model for evaluation on rank 0, use model_avg if available
        ...
        # Get rank and world size for DDP
        ...
        # Only set up evaluation on rank 0
        if args.dataset in ['kit', 'humanml'] and args.eval_during_training and self.rank == 0:
            mm_num_samples = 0
            mm_num_repeats = 0
        ...
        self.use_ddp = True
        if self.rank == 0:
            print(f"Setting up DDP with world_size={self.world_size}")
    ...
    def run_loop(self):
        ...
        if self.rank == 0 and (self.total_step() - 1) % self.save_interval != 0:
            self.save()
            self.evaluate()
    ...
    def update_average_model(self):
        # Update average model only on rank 0
        if self.args.use_ema and self.rank == 0:
